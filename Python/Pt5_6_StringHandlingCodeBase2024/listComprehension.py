...
print(f"The added items from listA {aList} and listB{bList}\n List AB:{listAB} ")
...
listCD = [cList[i] for i in range(len(cList)) if cList[i] in dList]
print(listCD)

eList = ["a","b","c"]
fList = ["c","d","e"]

listXY = [eList[i] for i in range(len(eList)) if eList[i] in fList]
print(listXY)
"Objective"
"" '' # Compare list comprehension with non list comprehension syntax 
